...
describe('MyComponent.vue', () => {
  ...
  beforeEach(() => {
    ...
    localVue.prototype.$route = { params: { /* Simuler les params du routeur ici */ }};
  ...
  it('montre Matrice lorsque showMatrice est true', () => {
    ...
    expect(wrapper.findComponent({ name: 'NetworkGraphComponent' }).exists()).toBe(false);
  });

  it('montre NetworkGraphComponent lorsque showMatrice est false', async () => {
    await wrapper.setData({ showMatrice: false });
    expect(wrapper.findComponent({ name: 'NetworkGraphComponent' }).exists()).toBe(true);
