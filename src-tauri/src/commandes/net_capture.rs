...
use crate::{
    errors::CaptureStateError, tauri_state::capture::{capture_config::CaptureConfig, capture_handle::CaptureHandle, capture_status::CaptureStatus, CaptureState},
...
pub fn config_capture(
    ...
    info!("[get_config_capture] app.config {:?}", app.config.device_name);
    info!("[get_config_capture] app.config {:?}", app.config.buffer_size);
    Ok(app.config.clone())
}
